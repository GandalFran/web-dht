...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      req.open('GET',process.env.SERVER_URL_PREFIX + ":" + process.env.SERVER_PORT + "/download/status",false);
      req.send();
    ...
    methods: {
      onButtonClick(item) {
        ...
        const req = new XMLHttpRequest();
        req.open('POST',process.env.SERVER_URL_PREFIX + ":" + process.env.SERVER_PORT + "/download/delete",false);
